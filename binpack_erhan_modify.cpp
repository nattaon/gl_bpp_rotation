...
void write_visualization_data_file(void)
{
 ...
 /*   sprintf(strcox, "%d", boxlist[cboxi].cox);
	...
	box_z[current_item] = boxlist[cboxi].coz;
	orien_x[current_item] = boxlist[cboxi].packx;
	orien_y[current_item] = boxlist[cboxi].packy;
	orien_z[current_item] = boxlist[cboxi].packz;
	bin_no[current_item] = 1;
  }
  else // unpackbox
  {
 /*   sprintf(n, "%d", cboxi);
	...
	box_z[current_item] = 0;
	orien_x[current_item] = boxlist[cboxi].dim1;
	orien_y[current_item] = boxlist[cboxi].dim2;
	orien_z[current_item] = boxlist[cboxi].dim3;
	bin_no[current_item] = -1;
  }
  current_item++;
...
void report_results(void)
{
 ...
 /* if ( (visualizer_file = fopen(graphout,"w")) == NULL )
  ...
  if ( (report_output_file = fopen(fileout,"w")) == NULL )
  {
